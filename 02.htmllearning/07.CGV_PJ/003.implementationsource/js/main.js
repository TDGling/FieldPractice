...
for (var i = 0; i < len; i++) {
    ...
    // 이퀄 오른쪽에 할당되는데 이때 chgMV() 함수를 직접쓰면 함수가 바로 실행됨
    // 따라서 이벤트 발생시 호출하려면 익명함수 즉, function(){} 안에 써야함
    // 이것은 마치 <a href="" onclick="chgMV()"> 라고 설정한것과 같다

    // 이벤트에 할당한 익명함수 안의 함수에 this를 보내면 이벤트가 걸린 요소자신이 함수에 전달된다.
...
function chgMV(ele){ // ele -> 전달되는 a요소
    ...
    // 3. 변경내용: 대상요소의 src속성을 변경
    // src값 중 동영상 ID만 변수에 담긴것으로 변경하여 반영
    tg.src = 
