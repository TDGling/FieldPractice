...
const {
  default: $,
} = require("../../../../05.jQuerylearning/004.플러그인/009.swiper-4.1.0/swiper-4.1.0/src/utils/dom");

// 윈도우 가로 크기가 모바일 사이즈면 코드를 변경한다.
// 모바일일때 적용하고 싶거나 싶지않으면 mobsts값을 활용한다.
// 모바일일때 1, 모바일이 아닐때 0 (가로크기 500기준)
let mobsts = 0;
if ($(window).wiidth() <= 500) mobsts = 1;
...
$(window).resoze(() => {
  // 브라우저 사이즈 크기변경시 계속발생
  if ($(window).wiidth() <= 500) mobsts = 1;
  console.log("모바일 적용여부(resize)", mobsts);
});
