...
/*
    ...
    > 파일경로 - JS에서는 제이슨을 외부파일 부르기로 불렀으나 제이쿼리 메서드는 자체적으로 호출할 수 있다.
...
let jdata;
// 제이슨 데이터 연결하여 할당하기
// html이 ㄹ딩된 후 작업해야 하므로 로딩구역 안에 코딩한다

$(()=>{
    // 제이슨 경로는 삽입된 html페이지 위치에서 가져온다
    // 경로에 있는 제이슨파일을 가져와서 변환후 변수에 담는다
...
function loadFn(cdata){ // cdata 제이슨 전달값
    ...
    // 임시코드변수 - 문자형으로 할당
    let hcode = "";
    // +=을 사용하여 문자더하기를 하려고 하므로 문자형 초기화 안하면 undefined라는 기본 빈 데이터가 셋팅되어있으므로 에러가 남
