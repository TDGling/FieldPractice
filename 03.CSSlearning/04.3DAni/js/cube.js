...
window.addEventListener("DOMContentLoaded",function(){
    ...
    const aniFn = function(){
        ...
        /* 
            ...
            1. add(클래스명) -> 클래스추가
            2. remove(클래스명)-> 클래스제거
            3. toggle(클래스명) -> 클래스 우무에 따라 추가 제거
            4, contains(클래스명) -> 자정된 클래스가 있으면 true(없으면 false)
        */
    }; 
    // 대상에 클릭설정하기
    btns.onclick = aniFn;
    // 전달값이 없다면 소괄호 생략한 함수명만 이벤트속성에 할당하면 바로 실행되지 않고 이벤트가 발생할때 실행된다.
});